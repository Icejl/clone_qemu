...
struct DeviceState {
	...
	// 是否已经调用过realized
    bool realized;
	// 判断销毁识别是否已经具体化，如果是，则发送DEVICE_DELETED事件
...
struct BusState {
    Object obj;
	// 总线的父设备，因为总线不能单独存在，只能依赖于设备，如PCI设备是有PCI桥产生的，
	// USB总线是有USB控制器产生的，SCSI总线是由SCSI控制器产生的。
    DeviceState *parent;
    char *name;
	// 热插拔处理函数
