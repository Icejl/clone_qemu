...
static void intel_hda_corb_run(IntelHDAState *d)
{
    ...
    for (;;) {
        ...
        verb = ldl_le_pci_dma(&d->pci, addr + 4*rp);		// dma_memory_read，读4个字节
        d->corb_rp = rp;
...
{
    type_register_static(&hda_codec_bus_info);	// hda总线
    type_register_static(&intel_hda_info);		// intel-hda-generic pci控制器
    type_register_static(&intel_hda_info_ich6);	// intel-hda pci控制器
    type_register_static(&intel_hda_info_ich9); // ich9-intel-hda pci控制器
    type_register_static(&hda_codec_device_type_info);	// hda-codec设备
