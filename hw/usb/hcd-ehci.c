                             ...
                             uint64_t val, unsigned size)
{
    ...
    case USBCMD:	// usb命令
        ...
        if (((USBCMD_RUNSTOP | USBCMD_PSE | USBCMD_ASE) & val) !=
            ...
            ehci_update_halt(s);
            s->async_stepdown = 0;
            qemu_bh_schedule(s->async_bh);	// 执行异步调度
        }
        break;
    ...
    case CONFIGFLAG:	// 配置标志
        val &= 0x1;
		// 如果是0，每个端口路由到companion controller (OHCI or UHCI)，为了兼容1.x
		// 如果是1，默认连接到ehci控制，如果连接设备 usb 1.x，则路由到指定的控制器
        if (val) {	
            for(i = 0; i < NB_PORTS; i++)
                handle_port_owner_write(s, i, 0);	// 重新attach
        }
        break;
	// CTRLDSSEGMENT(+10h) 没有处理，在ehci_caps_write HCCPARAMS bit0=1时，表示使用64位地址，这个就表示高32位地址
    case PERIODICLISTBASE:	// 周期性frame list地址，4k对齐
    ...
    case ASYNCLISTADDR:		// aync lsit地址
...
static int ehci_execute(EHCIPacket *p, const char *action)
{
    ...
    p->pid = ehci_get_pid(&p->qtd);		// 转换pid，0 = OUT(0xe1), 1 = IN(0x69), 2 = SETUP(0x2d)
...
static EHCIQueue *ehci_state_fetchqh(EHCIState *ehci, int async)
{
	...
	// 通过addr查找设备，设备挂载在posts上，addr从0开始，按顺序递增
    if (q->dev == NULL) {
...
static void ehci_advance_state(EHCIState *ehci, int async)
{
    ...
    do {
        ...
        case EST_WAITLISTHEAD:		// 异步起始，找到QH链表头，进入EST_FETCHENTRY
            again = ehci_state_waitlisthead(ehci, async);
            break;

        case EST_FETCHENTRY:		// 周期起始，判断结构类型，执行指定处理函数QH、iTD、siTD
            again = ehci_state_fetchentry(ehci, async);
            break;

        case EST_FETCHQH:			// QH，节点异常则到next QH，否则尝试使用qTD，无效则跳到EST_ADVANCEQUEUE
        ...
        case EST_FETCHITD:			// iTD，直接处理数据，完成后回到EST_FETCHENTRY
            again = ehci_state_fetchitd(ehci, async);
            itd_count++;
            break;

        case EST_FETCHSITD:			// siTD，功能还未实现
            again = ehci_state_fetchsitd(ehci, async);
            itd_count++;
            break;

        case EST_ADVANCEQUEUE:		// 尝试使用next qTD、altemate qTD，如果没有有效的，尝试下一个QH
            assert(q != NULL);
            again = ehci_state_advqueue(q);
            break;

        case EST_FETCHQTD:			// qTD，解析数据，把qTD结构赋给QH，后续使用QH中的qTD指针是来自qTD的
            assert(q != NULL);
            again = ehci_state_fetchqtd(q);
            break;

        case EST_HORIZONTALQH:		// 获取QH next，继续回到EST_FETCHENTRY消息
            assert(q != NULL);
            again = ehci_state_horizqh(q);
            break;

        case EST_EXECUTE:			// 开始数据包处理
            assert(q != NULL);
        ...
        case EST_EXECUTING:			// 数据处理完，检测完成状态，异常则到next QH，否则到EST_WRITEBACK
            assert(q != NULL);
            if (async) {
                ehci->async_stepdown = 0;
            }
        ...
        case EST_WRITEBACK:			// 通过token检测QH包状态，异常到next QH，否则到EST_ADVANCEQUEUE
            assert(q != NULL);
...
static void ehci_work_bh(void *opaque)
{
    ...
    if (ehci_periodic_enabled(ehci) || ehci->pstate != EST_INACTIVE) {
        ...
        for (i = 0; i < uframes; i++) {
            ...
            if ((ehci->frindex & 7) == 0) {
                ehci_advance_periodic_state(ehci);	// 周期性调用
            }
    ...
    if (ehci_async_enabled(ehci) || ehci->astate != EST_INACTIVE) {
        need_timer++;
        ehci_advance_async_state(ehci);			// 异步调用
    }
...
void usb_ehci_init(EHCIState *s, DeviceState *dev)
{
    ...
    memory_region_init_io(&s->mem_caps, OBJECT(dev), &ehci_mmio_caps_ops, s,	// 0 ~ 0x10 
                          "capabilities", CAPA_SIZE);
    memory_region_init_io(&s->mem_opreg, OBJECT(dev), &ehci_mmio_opreg_ops, s, //  0x20 ~ 0x64
                          "operational", s->portscbase);
    memory_region_init_io(&s->mem_ports, OBJECT(dev), &ehci_mmio_port_ops, s,	// 0x64 ~ 0x7c
                          "ports", 4 * s->portnr);
