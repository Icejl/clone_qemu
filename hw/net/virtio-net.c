                                      ...
                                      size_t size, bool no_rss)
{
    ...
    if (mhdr_cnt) {
		// CVE-2021-3748：当mergeable_rx_bufs开启时，mhdr_sg中保存了elem的iov地址，
		// 但在操作完后elem被umap（umap只会导致host中地址与qemu虚拟地址取消映射，但虚拟内存地址还是可以访问），再引用导致UAF
...
static int32_t virtio_net_flush_tx(VirtIONetQueue *q)
{
	...
	// 因为一个queue里面包含了tx_vq,rx_vq，所以索引要除2
    ...
    for (;;) {
        ...
        if (n->host_hdr_len != n->guest_hdr_len) {
            unsigned sg_num = iov_copy(sg, ARRAY_SIZE(sg),
                                       out_sg, out_num,
                                       0, n->host_hdr_len);
		   // 当out_sg中数据长度小于n->guest_hdr_len时，会触发断言
...
static void virtio_net_handle_tx_bh(VirtIODevice *vdev, VirtQueue *vq)
{
    ...
    virtio_queue_set_notification(vq, 0);
    qemu_bh_schedule(q->tx_bh);	// call virtio_net_tx_bh
}
...
static void virtio_net_device_realize(DeviceState *dev, Error **errp)
{
	...
	// 设置rx_vq,tx_vq handle函数
    for (i = 0; i < n->max_queues; i++) {
        virtio_net_add_queue(n, i);
    }

	// 注册了一个ctl控制函数，按顺序索引为2
    n->ctrl_vq = virtio_add_queue(vdev, 64, virtio_net_handle_ctrl);
	...
	// vnet_hdr默认开启，表示在接收大数据包时，使用部分checksum
