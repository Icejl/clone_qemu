...
int vhost_net_start(VirtIODevice *dev,  // virtio 网卡
					NetClientState *ncs,	// virtio对应的后端tap设备
                    int total_queues)		// 网卡队列数，默认为单队列，一个收包队列，一个发包队列
{
	...
	// 调用virtio总线类的回调函数，对应的函数是 virtio_pci_set_guest_notifier
	// 用来设置宿主机到虚拟机通知的eventfd的设置，最终在内核中生成一个irqfd，
	// 并通过VHOST_SET_VRING_CALL通知到vhost net模块
