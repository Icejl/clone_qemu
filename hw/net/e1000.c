...
xmit_seg(E1000State *s)
{
    ...
    if (tp->cptse) {
        ...
        DBGOUT(TXSUM, "frames %d size %d ipcss %d\n",
               frames, tp->size, css);
        if (props->ip) {    /* IPv4 */
            stw_be_p(tp->data+css+2, tp->size - css);	// total length
            stw_be_p(tp->data+css+4,
                     lduw_be_p(tp->data + css + 4) + frames);
        } else {         /* IPv6 */
            stw_be_p(tp->data+css+4, tp->size - css);
        }
		// tcp校验和起始位置，正常是0x22，跳过IP
        css = props->tucss;
        len = tp->size - css;
                 ...
                 unsigned size)
{
	...
	// 数组中的宏定义，是通过e1000x_common.h中定义的枚举量
...
static void pci_e1000_realize(PCIDevice *pci_dev, Error **errp)
{
	...
	// 将网卡接收函数注册到nc->incoming_queue队列的回调函数中
	// 当调用deliver函数qemu_deliver_packet_iov时，调用接收函数
