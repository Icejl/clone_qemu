...
struct VirtQueue
{
    ...
    /* Next head to pop */
	// 下一次从avail ring取数据的索引
    uint16_t last_avail_idx;
    bool last_avail_wrap_counter;

    /* Last avail_idx read from VQ. */
	// 最近一次从avail ring读取的idx
                    ...
                    unsigned int len, unsigned int idx)
{
	...
	// 本次使用avail ring指向desc的head index
    uelem.id = elem->index;
	// 使用了多少数据
    uelem.len = len;
...
void virtqueue_push(VirtQueue *vq, const VirtQueueElement *elem,
                    unsigned int len)	// len代表设备本次操作消耗了多少数据
{
    RCU_READ_LOCK_GUARD();
	// 构造一个VringUsedElem，并写到used ring数组中
...
static void *virtqueue_split_pop(VirtQueue *vq, size_t sz)
{
    ...
    RCU_READ_LOCK_GUARD();
	// last_avail_idx与avail ring中的idx相等则为空
	...
	// 通过last_avail_idx做索引，去avail ring的ring数组的取出索引
	// 做为descriptor ring的索引
    if (!virtqueue_get_head(vq, vq->last_avail_idx++, &head)) {
        goto done;
    }

	// 设置VIRTIO_RING_F_EVENT_IDX
	...
	// 读取第一个descriptor
    vring_split_desc_read(vdev, &desc, desc_cache, i);
	// 如果设置VRING_DESC_F_INDIRECT标识，则代码当前desc.addr指向一个desc链表
	// 然后依次从链表中读取desc结构
    if (desc.flags & VRING_DESC_F_INDIRECT) {
        if (!desc.len || (desc.len % sizeof(VRingDesc))) {
            virtio_error(vdev, "Invalid size for indirect buffer table");
                                 ...
                                 size_t vdev_size, const char *vdev_name)
{
	...
	// 去调用virtio_(xxx_device)_instance_init进行，设备初始化
